...
| T24 | Every graded answer holds: correct answers pause too (bare `{pause:true}` on the correct branch), verdict prominent above the reveal, Continue → advances (Enter falls through the frozen card) | fill fixture answered right → `waiting === "pause"`, `.pr-verdict-slot` ✓, `.pr-reveal.good`, dot `.hit`; Enter → summary | correct-answer-holds test |
| T25 | Dots review + first-attempt scoring: answered dots read hit/miss and click into a review of the recorded snapshot; retry re-runs for real but `rec.ok`, kb stats, and the round score keep the first attempt (dot shows the green solved-on-retry ring); the live card and the editor's round program survive the detour; wrong reveals link "🔬 step through this run"; 📝 notes persist (`plp.notes.v1`), Esc dismisses progressively | seeded round: miss Q1 → review via dot, wrong+right retries → `{ok:false, retry.ok:true}`, stats `{seen:1,missed:1}`, editor restored, `.pr-dot.miss.retried`; back → live ask intact; notes fill + Esc | dots-review test |
...
| T32 | Unified ← / one-level-up: ←/Esc from a round or summary reach the menu with the round DOM-stashed and resumable ("▶ Continue your round"); map → menu; menu → IDE; a map-launched round (`store.origin`) ends back on the map; open-in-editor / see-memory hops record a history entry | ← drive through round/map/menu; Continue restores the live ask; map-launched round ✕ → 7 lanes | unified-← test |
| T33 | Summary review discoverability: "🔍 Look back at the ones you missed" (only when misses exist, practice surface only) opens the first miss's review; ↩ Back restores the summary via the review stash; miss dots render larger with an ✗ glyph and a visible focus ring | seeded miss+skip round → summary button → review of Q1 → back → summary intact; `button.pr-dot.miss` present | summary-review test |
