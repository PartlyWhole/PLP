...
| T25 | Dots review + first-attempt scoring: answered dots read hit/miss and click into a review of the recorded snapshot; retry re-runs for real but `rec.ok`, kb stats, and the round score keep the first attempt (dot shows the green solved-on-retry ring); Previous/Next traverse review and restore the live card; wrong reveals link "🔬 step through this run"; 📝 notes persist (`plp.notes.v1`) and have a visible Collapse control while Esc remains progressive | seeded round: miss Q1 → review via dot, wrong+right retries → `{ok:false, retry.ok:true}`, stats `{seen:1,missed:1}`, editor restored, `.pr-dot.miss.retried`; latest-review Next → live ask intact; notes fill + Collapse + Esc | dots-review test |
...
| T26 | trace-table ask: silent trace builds the table (the trace is the answer key), non-empty gate, one lock grades every cell (container-forgiving) with per-cell ✓/✗ + truth beside misses, all-or-nothing round score, clean first-attempt all-correct grants met, review rebuilds the graded table (no retry widget) | two seeded inline-lesson drives: partial-wrong (no grant, stats miss) and all-correct (met granted, review table) | trace-table tests |
...
| T35 | `order-the-lines` (Parsons, ladder §R2): the deal is shuffled at COMPILE time from the round rng and never starts solved; `loadCode` carries the shuffled join; the widget replaces the program block (`program: false`) and moves rows with ↑/↓; grading EXECUTES the arrangement (any order that really prints the target is right, a raising order grades wrong); NO met grant in v1 while stats/score still move; review shows the arrangement + what it printed and the retry re-deals the rows and re-runs | seeded focus round (`state`/000A, seed 2): shuffled ≠ canonical, wrong order → `wrong` + real printed reveal + stats miss + met `{}`; right order → `correct` + met `{}`; review/retry roundtrip (`retry` takes an id array) keeps `rec.ok`; renderOrderLines unit checks (↑/↓, collect, freeze) | order-the-lines tests (T + Q) |
...
| T38 | `write-the-line` (ladder §R5): fill-one-blank with a line-wide blank — it RIDES the `fill-one-blank` ask kind (`form: "write-the-line"`), so grading is the same splice-run-compare; the dealt program shows a whole-line `___`, the intended line grades right, the plausible CONSTANT line grades WRONG (the §R5 scope rule made visible), a syntactically invalid line grades wrong without wedging the round, and curly quotes are normalized before the splice so the console reveal shows the straight-quoted program that really ran | derived seed (loops focus round on 001J, scanned for `write-loop-step`): four rounds — intended line → `correct` + console == target; `total = <final>` → `wrong`, every printed line the finished value; `total = total +` → `wrong`, waiting `pause`, no engine errors; `… + int(“0”)` → `correct` with `int("0")` in the editor | write-the-line test (T) |
...
| T36 | `predict-the-error` (ladder §R3): the card shows the numbered line picker + the FIXED four-name palette (`program: false`); grading runs it for real, requires `uncaught_exception`, and compares the pick with the engine's own `type_name` + raising line (`actions.lastException`, which merges the terminal record's type with the last `exception` step's location); both-right on the first attempt before the final hint GRANTS met; a half-right pick is named in the verdict but scores wrong; ungradable runs take the existing skip path; review marks the pick and the truth, retry re-picks and re-runs | seeded round: wrong line → `wrong` + reveal + stats miss + met `{}`; right line + right kind → `correct` + met granted; review/retry roundtrip (`retry` takes `{line, type}`) keeps `rec.ok`; renderErrorPicker unit checks (pick, collect, freeze, four names always) | predict-the-error tests (T + Q) |
...
| T37 | `predict-io` (ladder §R4a): the ask carries a `stdinScript`; the card SHOWS it as chips beside the program (the typing is scaffolded — where it lands is what is tested) and the answer widget is the growing line-box widget (T39); grading traces the program for real with each rendezvous auto-answered from the script (`runner.traceWithScript`, driven by the new `input-requested` event, answers deferred with `queueMicrotask`), then compares the whole console transcript, accepting the echo-stripped reading too; a first-attempt-correct answer before the final hint GRANTS met (binding §4); a script the program OUT-ASKS interrupts the run and takes the skip path — never left waiting (invariant 2); review re-shows the chips and the retry re-runs the same script | seeded focus round on 0026 (`state`, seed 0 = one rendezvous, seed 2 = two): chips rendered, mechanics line, real transcript with exactly one echo, wrong → stats miss + met `{}`, full transcript → `correct` + met granted, echo-stripped → `correct`; starved script → `skipped` + `terminal_reason: "interrupted"` + `isRunning()` false; review/retry roundtrip keeps `rec.ok` | predict-io tests (T) |
