...
| # | Feature | Best evidence | Coverage |
|---|---|---|---|
| T1 | Focus mode: Exercises hidden by default; the header toggle enters focus (stage up in the code column, editor receded, transcript behind 📜 History); visibility persists, focus flags never do | DOM: `#layout.focus` + stage visible across toggle + reload; history toggle shows the transcript pane; menu = welcome card + 9 controls | T-1 |
...
| T18 | Reveal choreography: locking a prediction grows the console strip (the "watch it run" cue); the explain beat keeps it; the next question resets; predict-state reveals open the memory pane | console rect grows >1.5× on lock, resets on next ask; `focus-memory` + visible #memory-pane after a predict-state lock | reveal choreography test |
