...
| Q13 | Progressive trace simulation preserves every raw executed-line occurrence while using memory line-position boundaries for produced state; loop exhaustion headers, final print output, and caller-resume attribution are explicit | while sequence includes final false header; one-line loop has four separate line-2 occurrences despite three line positions; function return is `L3 returns 6`, caller binding resumes at L4, next source line remains L5 | trace-simulation Q tests |

## Guided tutor (T-series = `tests/tutor.spec.mjs`)

| # | Feature | Best evidence | Coverage |
|---|---|---|---|
| T1 | Practice surface: Exercises hidden by default; the header toggle opens the full-viewport card view (`body.practice` hides header + IDE); visibility persists; the labeled surface control leaves for its stated destination | DOM: `body.practice` + `#practice` visible across toggle + reload; header/#layout hidden; menu = welcome card + controls | T-1 |
...
| T23 | The card is the interface: program in-card (read-only CM), one-line prompt, correct-answer reveal from the graded run, wrong-answer correction on the same card, 🌱 `ask.teach` statement + collapsed example, `ask.context` for spot-diff, first-time-per-form mechanics line (once, `plp.practice.v1`), open-in-editor + restore chip, reload rebuilds the card | practice-DOM assertions across the migrated drill tests + edge-flow test | T-series practice tests |
| T24 | Every graded answer holds: correct answers pause too (bare `{pause:true}` on the correct branch), verdict prominent above the reveal, the canonical header Next advances, and Enter falls through the frozen card to Next | fill fixture answered right → `waiting === "pause"`, `.pr-verdict-slot` ✓, `.pr-reveal.good`, dot `.hit`; no bottom Continue; Enter → summary | correct-answer-holds test |
| T25 | Dots review + first-attempt scoring: answered dots read hit/miss and click into the recorded snapshot; unrevealed misses hide truth and trace links; retry re-runs for real but `rec.ok`, kb stats, mastery, and the round score keep the first attempt (dot shows the green solved-on-retry ring); Previous/Next traverse review and restore the live card; 📝 notes persist (`plp.notes.v1`) and have a visible Collapse control while Esc remains progressive | seeded round: miss Q1 → wrong+right retries → `{ok:false, retry.ok:true}`, stats `{seen:1,missed:1}`, editor restored, `.pr-dot.miss.retried`; hidden review has no expected/actual/partial truth; latest-review Next → live ask intact; notes fill + Collapse + Esc | dots-review + correction-lifecycle tests |
| T42 | Retry-before-reveal contract: the first wrong answer is counted exactly once and persisted as `disclosure: "hidden"`; the correction offers Try again, Reveal answer, and Next problem; wrong retries return only `{ok:false}` and disclose no truth, actual output, partial marks, or variant explanation; a correct retry persists in `rec.retry`, waits for Next, and never changes the first-attempt score/stats/mastery; Reveal answer changes disclosure without scoring; hidden Next advances with `wrong-unrevealed`; reload restores correction idempotently; legacy records without `disclosure` remain revealed | wrong → reload → wrong retry → reveal; wrong → hidden Next; wrong → correct retry → reload → Next; assert records/stats/score/mastery at every boundary, no answer-bearing DOM before reveal, authored explanation only after solve/reveal, and historical unrevealed review remains hidden | correction-lifecycle tests |
| T41 | Practice-player controls: header states `Question N of M`; labeled leave controls go directly to Topics/map/code; an unresolved live question has no Next and retains explicit Skip; a frozen answer advances only through header Next; Finish round is separated and confirmed; the menu persists 5/10/15/Custom (1-50) and applies the preference to regular/endless/focus rounds, with a visible notice for the two-problem single-template focus cap; 360px layout does not overflow | menu drive 10 → 5 → 15 → Custom 7, assert `plp.practice-count.v1`; regular and endless compile 7; single-template focus compiles 2 plus notice; frozen/live header assertions; dismiss then accept Finish confirmation; review-leave goes directly to Topics; 360px `scrollWidth <= innerWidth` | round-count + reveal + dots-review + labeled-leave tests |
| T26 | trace-table ask: silent trace builds the table (the trace is the answer key), non-empty gate, one lock grades every cell privately (container-forgiving), all-or-nothing round score, clean first-attempt all-correct grants met; a wrong first attempt and wrong retries show no per-cell marks or truth, while explicit Reveal answer rebuilds the fully graded table | two seeded inline-lesson drives: partial-wrong (no grant, stats miss, no partial feedback until reveal) and all-correct (met granted, review table) | trace-table + correction-lifecycle tests |
| T43 | KB trace-table forms run as progressive trace simulations: next-line then effects, verified-history-only ledger, private retries/current-phase reveal, exact phase reload, first-attempt score, and a fully concealed silent oracle | inline two-line trace: wrong line/effects expose no truth; line/end choices have mutually exclusive `aria-pressed`; corrected/revealed rows retain provenance; reload resumes effects with miss preserved; invalid saved bounds preserve the counted outcome but reset derived truth; persisted progress has no future sequence/signature; confirmed Finish cannot erase or double-count a miss; Finish during the trace cannot resurrect the ask; runner records/summary, memory records, transcript, and xterm buffer remain empty while active while cached stream validation stays clean; review renders exactly saved committed rows | trace-simulation tutor tests + K-runtime |
| T44 | Progressive trace uses a stable coding-puzzle board: direct source selection, compact verified history, explicit before/after effects and No visible effect, function context, repeated-line counts, completion route, keyboard operation, responsive stacking, and reduced-motion behavior | source button identity survives next-line → effects; untouched effects are incomplete without scoring a miss; No visible effect is mutually exclusive with changed names; committed line gets ✓ and collapsed history; loop line reaches ×2; verified call changes context to `double()`; completion route is `L1 › L2 › END`; arrow keys move source focus; 360px board stacks without overflow; reduced-motion transition is 0s | trace-simulation tutor UI tests |
| K-order | Order-matters variation discipline: 13 spot-the-difference siblings (state/strings/lists/structures/loops) where one moved line changes the result; A-output ≠ B-output on every seed, one-line (or flagged multiline) contract, footprint ⊆ contract on both pair sides | K-5/K-10/K-oracles over the grown pool (40 seeds/exercise real-execution verification at build time); design §5.5 | K-series (automatic) |
| T35 | `order-the-lines` (Parsons, ladder §R2): the deal is shuffled at COMPILE time from the round rng and never starts solved; `loadCode` carries the shuffled join; the widget replaces the program block (`program: false`) and moves rows with ↑/↓; grading EXECUTES the arrangement (any order that really prints the target is right, a raising order grades wrong); NO met grant in v1 while stats/score still move; a miss hides canonical order and actual output, while retry re-deals the rows and re-runs without changing `rec.ok` | seeded focus round (`state`/000A, seed 2): shuffled ≠ canonical, wrong order → hidden correction + stats miss + met `{}`; right order → `correct` + met `{}`; wrong retry returns only `{ok:false}`; explicit reveal shows canonical order/output; renderOrderLines unit checks (↑/↓, collect, freeze) | order-the-lines + correction-lifecycle tests (T + Q) |
| K-ol | order-the-lines form contract: the canonical join runs clean, prints exactly `targetOutput` on one line, and stays inside the closure; wrong arrangements do NOT print the target (discrimination by construction) | K-10 order branch (canonical + two explicit dependency-breaking permutations: reversed, first two swapped); footprintSources canonical join; docgen renders the canonical lines + output | K-10, K-5, K-doc |
| T38 | `write-the-line` (ladder §R5): fill-one-blank with a line-wide blank. It RIDES the `fill-one-blank` ask kind (`form: "write-the-line"`), so grading is the same splice-run-compare; the dealt program shows a whole-line `___`, the intended line grades right, the plausible CONSTANT line grades WRONG (the §R5 scope rule made visible), a syntactically invalid line enters correction without wedging or revealing the intended line, and curly quotes are normalized before the splice | derived seed (loops focus round on 001J, scanned for `write-loop-step`): intended line → `correct` + console == target; constant or invalid line → hidden correction, no answer/actual output, no engine errors; explicit reveal discloses the intended line; curly-quote input grades with `int("0")` in the editor | write-the-line + correction-lifecycle tests (T) |
| K-wl | write-the-line form contract: the blanked line's intended content runs clean and prints exactly `targetOutput` (multi-line by design — the running value on every pass), stays inside the closure, and the generator's `constantLine` (the plausible gaming answer) does NOT reproduce the target | K-10 write-the-line branch (intended splice + constantLine mustMissTarget); footprintSources default; docgen renders the filled program + real output | K-10, K-5, K-doc |
| T40 | `fix-the-bug` (ladder §R5's composition, never a third grading path): the program RUNS CLEAN but prints the wrong thing; the card states the intended output beside the real one and RIDES the `fill-one-blank` ask kind (`form: "fix-the-bug"`) with a RUNTIME-CHOSEN blank — the learner taps the line (`renderLinePicker`, now shared with predict-the-error, `program: false`) and writes its replacement in an always-EMPTY box (never the buggy line — E5); grading splices at the PICKED line and compares the real output with the intended one, so a repair on a DIFFERENT line that still prints it is CORRECT ("Not the line I'd have changed, but it works!") and the pick is never graded separately | derived seeds (focus rounds scanned for the template): `fix-accumulator` — buggy program dealt whole (no `___`), picker rows == program lines, no second program block, intended repair → `correct` + console == target, the gaming constant → `wrong`; `fix-alias` — line 3 rebuilt with `b = a + [v]` → `correct` with the warm verdict; review shows the picker with the picked line marked and `line N → <text>`, retry re-picks and re-runs (`retry` takes `{line, text}`) and keeps `rec.ok` | fix-the-bug tests (T) |
| K-fb | fix-the-bug form contract: the BUGGY program runs clean and really prints the recorded `wrongOutput` (which must differ from the intended output — E6); the intended fix spliced at `buggyLine` prints `targetOutput`; the plausible conceptless `constantLine` does NOT; and BOTH programs (buggy and fixed) satisfy the closure — which is why `fix-alias` focuses 0024 (slice-copies) rather than 000H: the repair `b = a[:]` emits 0024, a CHILD of 000H | K-10 fix-the-bug branch (three programs: buggy→wrongOutput, fixed→target, constantLine mustMissTarget) + the wrong≠intended assertion; footprintSources returns both sides; docgen renders the buggy program with its real output and the fixed program with the intended one | K-10, K-5, K-doc |
| T36 | `predict-the-error` (ladder §R3): the card shows the numbered line picker + the FIXED four-name palette (`program: false`); grading runs it for real, requires `uncaught_exception`, and compares privately with the engine's own `type_name` + raising line (`actions.lastException`); both-right on the first attempt before the final hint GRANTS met; a half-right pick scores wrong without identifying which half or showing the truth; ungradable runs take the existing skip path; retry re-picks and re-runs, and explicit reveal marks the pick and truth | seeded round: wrong line → hidden correction + stats miss + met `{}`; wrong retry returns only `{ok:false}`; Reveal answer shows type/line; right line + right kind → `correct` + met granted; renderErrorPicker unit checks (pick, collect, freeze, four names always) | predict-the-error + correction-lifecycle tests (T + Q) |
| K-pe | predict-the-error form contract: the program really raises the DECLARED type on the DECLARED line, prints ≤1 line before the crash, and its expectRaise footprint (up to and including the raise) stays inside the closure; the analyzer's raiseKind maps to the declared type's concept; the reference records only `Type (line N)` — never message text, whose wording drifts between system python3 and Pyodide | K-5 raise branch + `footprint(src, {expectRaise:true})`; K-5a anchors (all three detections, with and without the option); K-10 raise branch reading `plp.records()`; K-oracles parse-only probe + crash-rendering wrongAnswer floor; docgen `expectError` samples from both writers | K-5, K-5a, K-10, K-oracles, K-doc |
| T37 | `predict-io` (ladder §R4a): the ask carries a `stdinScript`; the card SHOWS it as chips beside the program and the answer widget is the growing line-box widget (T39); grading traces the program for real with each rendezvous auto-answered from the script (`runner.traceWithScript`), then compares the whole console transcript, accepting the echo-stripped reading too; a first-attempt-correct answer before the final hint GRANTS met; a wrong transcript enters correction without showing actual output; a script the program OUT-ASKS interrupts the run and takes the skip path; retry re-runs the same script privately | seeded focus round on 0026: chips rendered, real transcript with exactly one echo, wrong → hidden correction + stats miss + met `{}`, full or echo-stripped transcript → `correct` + met granted; wrong retry exposes no actual output; explicit reveal shows transcript; starved script → `skipped` + `terminal_reason: "interrupted"` + `isRunning()` false | predict-io + correction-lifecycle tests (T) |
| T39 | The growing line-box answer widget (`question-ui.createLinesInput`) replaces the textarea on every SEVERAL-line ask (predict-output with `ask.multiline`, predict-io, and their review retries) — the defect it fixes is that a textarea had NO keyboard submit at all (Enter-submits was bound only for `ask.singleLine`) while the mechanics line claimed "press Enter"; exactly ONE box renders to start (the count must never hint at how many lines print), Enter on a non-empty last box grows, Enter on a non-last box moves, Enter on an EMPTY last box (≥2) drops it and SUBMITS, Backspace merges upward, arrows walk, paste splits on newlines, a ✕ per extra box + "+ another line" keep the touch path whole; `collect()` joins with `"\n"` after dropping trailing empties, so GRADING is byte-identical and `lockPrediction("a\nb")` still fills it; the first-time mechanics line is keyed off the WIDGET built (`multiline`), not the form name | Q unit: one box to start, grow/move/submit/backspace/arrows/paste, collect joins + drops trailing empties, applyResult ok/bad, freeze disables; T end-to-end: scanned multi-line loops deal typed as two boxes and submitted with double-Enter grades against the real run, mechanics line no longer claims plain Enter submits, `lockPrediction` roundtrip grades correct | createLinesInput test (Q) + multi-line predict-output test (T) |
| K-io | predict-io form contract: the program runs clean under its script, the script length EXACTLY matches the number of rendezvous (no leftover line, no starvation), the transcript is gradable, and the footprint of the program stays inside the closure (the typed lines are outside data, never source); the reference records the ECHO-EXCLUDING transcript — CPython with piped stdin never echoes, so `console.engineText()` and `python3 < script` agree byte for byte — plus a labeled "someone types:" line | K-10 predict-io branch via the new `plp.traceWithStdin(lines)` helper (asserts `used === script.length` and `exhausted === false`); footprintSources → `[prog.code]`; K-oracles parse-only probe (the program cannot run without stdin); docgen `stdin` samples from both writers | K-10, K-5, K-oracles, K-doc |
| K-tt | trace-table form contract: generated programs yield 2..maxBlanks blanks, every watched name blanked ≥1×, expected values single-line, probed reads analyzer-visible | K-10 trace-table branch + footprintSources probe appends; docgen renders end-holdings | K-10, K-doc |
| T27 | Endless mode + score tracker: chunks auto-chain with no summary between; session score (answered/right/streak/best, first-attempt basis) carries across chunks and shows as the top-bar chip; reviews reach back across chunk boundaries; ✕ earns the whole-run summary with "∞ Go again"; menu shows the lifetime line and the ∞ entry | seeded endless drive: right answer → chip ✓ 1/1, chain → fresh lessonId with no summary, skip → streak 0, cross-chunk review, exit summary "1 of 2", menu "All time:" | endless test |
| T28 | No brain-dead cells, no stale repeats: trace-table literal binds render as givens (blanks are only COMPUTED changes; <2 real blanks → the builder refuses); templates answered right fade from selection (1/(1+0.75·right)); mastered concepts (met + clean practice) keep 25% weight; intros yield to reviews once a concept is familiar | Q trace-table givens/thin-null assertions; T "proven material recedes" property (template + mastery tallies over 8 seeds) | Q + T tests |
...
| T21 | Concept map: 7 lanes cover every non-structural concept once; cold-start frontier is exactly print-text; met/frontier/locked chips render; a frontier chip's Practice-this starts a targeted round (concept's own exercises, 4 questions, `drill-{topic}-{tag}-{seed}`); locked chips recommend but never forbid — "First: …" links plus a "Try it anyway ▶" jump-ahead | pure `mapModel` invariants + deterministic double-build; stage DOM chip-state counts; targeted-round store shape; locked-chip detail shows prerequisites + jump-ahead button | concept map test |
...
| T29 | History routing: surfaces hash-route (`#learn`/`#learn/round`/`#learn/map`/`#lesson`); pushState per user-level transition, no entries from endless chaining; popstate routes through the existing handlers; Back walks round → menu (round resumable) → IDE, Forward resumes the live card; reload keeps the hash and the hash restores the view; a first visit to `#learn` survives the COI-shim reload; `#room=` stays collab-owned | UI-driven push transitions + `page.goBack()/goForward()` assertions on view + live round state; first-visit `#learn` COI test | history + COI-hash tests |
| T30 | Continue signal: a persisted mid-round drill badges the Learn segment and shows a dismissible chip under the header; Continue takes the toggle path; both vanish the moment the surface opens; dismissal is session-only | persisted round + reload → badge + chip; Continue → `body.practice`, hint gone, `waiting === "ask"`; ✕ dismiss removes both | continue-signal test |
| T31 | World switch: top-left `[⌨ Code] [🌱 Learn]` segments with `aria-pressed` active states (`#btn-tutor` keeps its id on Learn); a temporary `[📖 Lesson]` segment appears while a guided lesson is live, resumes the stage, and disappears on exit | segment attribute assertions across start → Code-click → Lesson-click → exit | world-switch test |
| T32 | Labeled leave and Esc: Topics leaves round/review for the menu with the round DOM-stashed and resumable ("▶ Continue your round"); Back to topics leaves the map; Back to code leaves the menu; Esc progressively closes notes/review before moving up; a map-launched round (`store.origin`) finishes back on the map; open-in-editor / see-memory hops record a history entry | labeled controls drive through round/map/menu; Continue restores the live ask; a dismissed Finish confirmation keeps the ask and an accepted one returns a map-launched round to the lanes | labeled-leave test |
| T33 | Summary review discoverability: Previous opens the latest answered question; Previous/Next traverse answered reviews and latest Next restores the summary; miss dots render larger with an ✗ glyph and a visible focus ring | seeded miss+skip round → summary → Previous opens Q2 → Previous Q1 → Next Q2 → Next restores summary; `button.pr-dot.miss` present | summary-review test |
| T34 | Chrome minors: ⤢ swaps to ⤡ "Restore (Esc)" while a pane is maximized; an untraced Run leaves the memory pane explaining itself ("press Trace to watch names bind"); the stage header carries a visible "✕ End lesson" | maximize glyph/title round-trip; `memory-empty` text after `plp.run()`; stage-exit click ends the lesson | chrome-minors test |
...
| K19 | Challenge role (R1.2): braids ⊆ assumed, disjoint from the focus lineage, relaxed closure scoped to the role; availability = pool FILTER gated on focus + all assumed met; empty-met compiles deal none (fixture stability); challenges never offerable without their met focus (grantMet no-op) | static braid/closure audit; met-saturated compiles deal challenges with floors intact; braid removal from met removes exactly its dependents | K-4 + K-chal + K-8 |
| K20 | Hard siblings (R1.3): `difficulty: "hard"` legal on review/challenge only; filtered until met(focus); exempt from mastery ×0.25; half-rate template fade | schema audit + empty-met absence + share-rise property | K-4 + K-chal + "proven material recedes" (T) |
| K21 | Designed misconceptions (R1.1): rng-free per-instance `misconception` never equals the graded truth (printed output / probed value / program B's output), single-line under the multiline law | real-execution comparison on stratified samples; pure 40-seed string/shape audit | K-oracles + K-10 + K-mc |
| K23 | trace-query form (design/new-forms.md §1): the answer is DERIVED from the real trace (runs-count / last-line / value-when), never authored; single-line, stable across builds, positive counts, misconception ≠ derived answer on every stratified seed; no met grant in v1 | K-10 trace-query branch (in-engine derivation), Q-series generator test (counts, last-line, produced-state value-when, fail-closed on unknown name/visit), T-series e2e (correct grades right, met stays empty, stats bump) | K-10, Q-series, T-series |
| K22 | Functions sub-graph, def/call/return half (ladder §R4b A1/A2 + waves 1–3): top-level-only `def` with bare-name params and STRAIGHT-LINE bodies; calls parsed for any name with footprint as the judge (user function / unbound → would-raise / unsupported builtin → unmapped-syntax); per-call abstract frames (params bound left to right, local-first read-through, writes always local, pre-scanned read-before-local-assign → would-raise); an UN-CALLED body emits nothing; return/consumed-result rules 002A/002B/002C/002G/002H | K-5b anchors (11 positive rule→tag cases + the scope/negative set: return at top level, def inside an if, call before def, UnboundLocal, non-callable, unsupported builtin, control flow in a body, a call from inside a body, redefinition, function-as-value); the 13 functions exercises pass the ordinary K-5/K-10/K-oracles/K-inv13/K-inv14 gates; `hasCompound` gains "def" (inv-8 ast diff skipped, same precedent as if/for/while) | K-5b, K-5, K-10, K-oracles, K-doc |
| K23 | Trace-table attribution over a call is PINNED, not assumed: a module binding produced by `x = double(v)` lands on a row labelled with the CALLEE's line, so wave-3's `two-calls-chain` table is deferred with frame-aware tables (ladder §R4b decision) | live trace-table build over a two-call chain asserts the exact rows (values right, line = the callee's `return` line) | K-fnattr |
...
| KT7 | Misconception follow-ups (R1.1): a wrong answer matching the ask's designed misconception bumps `plp.kb.mc.v1` (keyed misconceptionOf ?? concept); both compile sites thread the store; the compiler reserves a deterministic follow-up slot (contrast preferred, guards intact, merged with the worst slot on tag collision); resolving a followUp ask settles the entry; `quiz-graded` carries `misconception: bool` | end-to-end: designed wrong answer → store bump → next compile deals followUp → correct answer clears it; compile-level slot/guard/determinism properties | misconception T-tests |
...
- **X0b — deterministic replay.** Feeding the same deltas after `term.reset()`
  ...
  valid at all in an emulator.) A reset-generation probe also queues stale
  output before reset and current output after it, then requires both the
  authoritative store and settled buffer to contain only the current output.
