...
| E10d | **fix-the-bug** (ladder §R5's composition) is that same splice-run-compare with the splice target chosen at ANSWER time: the program RUNS CLEAN but prints the wrong thing, the card states the intended output beside the real one, and the learner taps the line then writes its replacement. Rules: both the BUGGY and the FIXED program satisfy the closure (the repair is usually where the focus fires); the recorded `wrongOutput` must differ from the intended output (E6) and is verified against real execution; the answer box is never pre-filled with the buggy line (E5); and the SCOPE RULE (E10c) carries over — the buggy line must execute more than once or feed ≥2 later observations, with `constantLine` recording the plausible conceptless answer that must MISS. In a loop-HEADER slot no constant can hold the loop at all, so the recorded shortcut there is `range(n)` ("count the lines you need"), which misses because the intended range starts at ≥2; enumerating the intended values instead is a legitimate repair, since producing them requires the same diagnosis | ladder §R5 | K-10 fix-the-bug branch (buggy→`wrongOutput`, intended fix→target, `constantLine`→must MISS, wrong≠intended), footprintSources on both sides, T-series fix-the-bug tests |
