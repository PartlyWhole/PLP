...
| R4b (waves 1–3) | Functions: def/call/return | **DONE** | Analyzer pieces A1+A2: top-level-only `def` grammar (bare-name params, straight-line bodies), `return` inside a body, the parseAtom builtin gate relaxed so footprint judges any called name, per-call abstract frames (left-to-right param binding, local-first read-through, local-only writes, pre-scanned read-before-local-assign → would-raise), and the rule→tag table 0027/0028/0029/002F (A1) + 002A/002B/002C/002G/002H (A2). Concepts moved pending → `kb/concepts/functions.mjs`; 13 exercises in `kb/exercises/functions.mjs`; new topic "Functions". Anchors: K-5b (11 positive cases + 10 negatives), K-fnattr. **DEVIATION**: wave 3's `two-calls-chain` trace table is NOT shipped — the attribution probe showed a module binding produced by a call is attributed to the CALLEE's line, so the table would mislabel its rows; K-fnattr pins that behavior and the exercise waits with frame-aware tables. 002D/002E (wave 4) and 002J (wave 5) remain in `kb/concepts/pending.mjs`. |
